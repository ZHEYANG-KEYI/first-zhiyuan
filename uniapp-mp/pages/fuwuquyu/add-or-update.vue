	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.fuwuquyu" v-model="ruleForm.fuwuquyu" placeholder="服务区域"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fuwuquyu`, this.ruleForm.id);
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.fuwuquyu)){
					this.$utils.msg(`服务区域不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`fuwuquyu`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`fuwuquyu`, this.ruleForm);
						}else{
							await this.$api.add(`fuwuquyu`, this.ruleForm);
						}
