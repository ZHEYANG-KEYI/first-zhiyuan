		...
		<view :style='{"width":"100%","padding":"0 0 40rpx","position":"relative","background":"#dfd8cc","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #fafafa","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.peixunmingcheng" type="text" placeholder="培训名称" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing07" :style='{"color":"rgb(153, 153, 153)","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"2rpx solid #fafafa","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.zhiyuanzhezhanghao" type="text" placeholder="志愿者账号" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0","background":"#DFD8CC","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 4rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.peixuntupian)" :src="product.peixuntupian.split(',')[0]"></image>
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.peixuntupian?baseUrl+product.peixuntupian.split(',')[0]:''"></image>

					<view class="listmr" :style='{"width":"calc(100% - 260rpx)","padding":"0","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#000000"}'>{{product.peixunmingcheng}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('peixunchengji','修改')) || (!userid && isAuthFront('peixunchengji','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('peixunchengji','删除')) || (!userid && isAuthFront('peixunchengji','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('peixunchengji','修改')) || (!userid && isAuthFront('peixunchengji','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('peixunchengji','删除')) || (!userid && isAuthFront('peixunchengji','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.peixunmingcheng="";
				this.searchForm.zhiyuanzhezhanghao="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.peixunmingcheng){
					params['peixunmingcheng'] = '%' + this.searchForm.peixunmingcheng + '%'
				}
				if(this.searchForm.zhiyuanzhezhanghao){
					params['zhiyuanzhezhanghao'] = '%' + this.searchForm.zhiyuanzhezhanghao + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`peixunchengji`, params);
                } else {
                    res = await this.$api.list(`peixunchengji`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('peixunchengji', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.peixunmingcheng){
					searchForm['peixunmingcheng'] = '%' + this.searchForm.peixunmingcheng + '%'
				}
				if(this.searchForm.zhiyuanzhezhanghao){
					searchForm['zhiyuanzhezhanghao'] = '%' + this.searchForm.zhiyuanzhezhanghao + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`peixunchengji`, searchForm);
                } else {
                    res = await this.$api.list(`peixunchengji`, searchForm);
                }
				// 如果是第一页数据置空
