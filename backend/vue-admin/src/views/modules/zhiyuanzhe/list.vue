<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('zhiyuanzhe','服务统计')" @click="fuwutongjiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<fuwutongji-cross-add-or-update v-if="fuwutongjiCrossAddOrUpdateFlag" :parent="this" ref="fuwutongjiCrossaddOrUpdate"></fuwutongji-cross-add-or-update>
...
import fuwutongjiCrossAddOrUpdate from "../fuwutongji/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				fuwutongjiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fuwutongjiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwutongjiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zhiyuanzhe');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fuwutongjiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fuwutongjiCrossaddOrUpdate.init(row.id,type);
				});
			},
