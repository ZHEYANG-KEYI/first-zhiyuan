		...
		<template v-if="showFlag">
			...
			<div :style='{"width":"100%","padding":"0","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="xuanchuantupian" width="200" label="宣传图片">
						<template slot-scope="scope">
							<div v-if="scope.row.xuanchuantupian">
								<img v-if="scope.row.xuanchuantupian.substring(0,4)=='http'" :src="scope.row.xuanchuantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.xuanchuantupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.xuanchuantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.xuanchuantupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="huodongshipin" label="活动视频">
						<template slot-scope="scope">
							<el-button v-if="scope.row.huodongshipin" type="text" size="small" @click="preClick(scope.row.huodongshipin)">预览</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
...
import AddOrUpdate from "./add-or-update";
	...
	huodongxinxistatusChange(e,row){
		if(row.status==0){
